...
def upgrade() -> None:
    ...
    with op.batch_alter_table("generated_images") as batch_op:
        ...
        batch_op.create_foreign_key("fk_generated_images_user_id", "users",["user_id"], ["id"],)
        batch_op.create_index("ind_generated_images_user_id", ["user_id"], unique=False)

def downgrade() -> None:
    """Downgrade schema."""
    with op.batch_alter_table("generated_images") as batch_op:
        batch_op.drop_index("ix_generated_images_user_id")
        batch_op.drop_constraint("fk_generated_images_user_id_users", type_="foreignkey")
